...
static unordered_map<const TripAnalyzer*, TripData*> storage;

static TripData* getData(const TripAnalyzer* ta) {
    auto it = storage.find(ta);
    if (it == storage.end()) {
...
static inline void chew(TripData* data, const char* lb, const char* le, bool& bomDone, bool& headerDone) {
    if (le > lb && le[-1] == '\r') --le;
    if (le <= lb) return;
    ...
    auto it = data->zones.try_emplace(std::move(key), ZoneStats{}).first;
    ++it->second.total;
    ++it->second.byHour[hr];
}
...
void TripAnalyzer::ingestFile(const string& csvPath) {
    ...
    TripData* data = getData(this);
    data->zones.clear();
    data->zones.reserve(200000);
    data->zones.max_load_factor(0.7f);
    ...
    while (true) {
        ...
        while (p < end) {
            ...
            if (!carry.empty()) {
                ...
                chew(data, carry.data(), carry.data() + carry.size(), bomDone, headerDone);
                carry.clear();
            } else {
                chew(data, p, nl, bomDone, headerDone);
            }
            p = nl + 1;
        }
    }

    if (!carry.empty()) chew(data, carry.data(), carry.data() + carry.size(), bomDone, headerDone);
    fclose(f);
}
