...
static unordered_map<string, ZoneStats> zones;
...
static inline void chew(const char* lb, const char* le, bool& bomDone, bool& headerDone) {
    if (le > lb && le[-1] == '\r') --le;
    if (le <= lb) return;
    ...
    while (true) {
        ...
        while (p < end) {
            ...
            if (!carry.empty()) {
                ...
                chew(carry.data(), carry.data() + carry.size(), bomDone, headerDone);
                carry.clear();
            } else {
                chew(p, nl, bomDone, headerDone);
            }
            p = nl + 1;
        }
    }

    if (!carry.empty()) chew(carry.data(), carry.data() + carry.size(), bomDone, headerDone);
    fclose(f);
}
